...
11. **PR 合并后，用网页端 GPT 的 apps 功能做 Tech Lead 总结**
   ...
   ```text
   当前 PR XX （对应issue XX） 已经完成。请先站在 tech lead 视角详细评估，给出这个pr的影响力，在其前后这个项目分别是怎么样的。然后好好介绍一下本次开发的目的，过程，遇到了哪些困难，如何克服的，最终目标达成了没有，如何达成的。用简单明了的大白话，图文并茂的，举例子的说明，读者是没读过这个项目代码的程序员。先建立业务能力清单，再分别还原新旧实现，用逐项对照确认覆盖范围。流程图是检查和讲解的骨架，不是文章写完后的插图。关键流程图须核对实际读写对象、调用顺序、条件分支和恢复路径；最终正文可以按读者更容易理解的顺序组织，不必机械重复全部核查步骤。
   ```
