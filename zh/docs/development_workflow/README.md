...
8. ** codex 负责代码审核**
   ...
   ```text
   对 XX 项目的 PR XX（最新head XX）进行严格详细全面的代码审查。在评估代码时不但要评估开发是否符合issue，还要评估有没有过度开发，是否可以在架构层级精简（功能可以提前开发，但是不允许有脱裤子放屁的冗余）。PR_BODY.md 是你重要的参考材料，你需要检查有没有重复开发和修补，如果有，分析其原因。重要问题需要实际运行代码来验证你的猜想，没有调查就没有发言权。
   ...
   * TESTING.md
   ...
   6. 在评估代码时不但要评估开发是否符合issue，还要评估有没有过度开发，是否可以在架构层级精简（功能可以提前开发，但是不允许有脱裤子放屁的冗余）。
...
9. **如果 review 有问题，先验证问题是否真实存在，再决定是否修**
   ...
   4. 在评估代码时不但要评估开发是否符合issue，还要评估有没有过度开发，是否可以在架构层级精简（功能可以提前开发，但是不允许有脱裤子放屁的冗余）。
