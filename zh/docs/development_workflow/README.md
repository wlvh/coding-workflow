...
9. **Codex 负责代码审核**
   ...
   ```text
   对 PR XX（最新 head XX）进行严格详细全面的代码审查。在评估代码时不但要评估开发是否符合 issue，还要评估有没有过度开发，是否可以在架构层级精简（功能可以提前开发，但是不允许有脱裤子放屁的冗余）。切记不要去优化或修复一个本不应该存在的问题！仓库外临时 PR body 是重要参考材料，你需要检查有没有重复开发和修补，如果有，分析其原因。重要问题需要实际运行代码来验证你的猜想，没有调查就没有发言权。
...
11. **PR 合并后，用网页端 GPT 的 apps 功能做 Tech Lead 总结**
   ...
   ```text
   当前 PR XX （对应issue XX） 已完成。请站在技术负责人视角，基于实际源码与开发记录，制作一份可离线阅读的 HTML，面向没读过项目代码的程序员，讲清项目在改动前后分别是什么样、本次开发的目的与影响、经历的困难与取舍，以及最终目标是否达成。
我希望读完后，既理解这次开发为什么值得做，也真正理解它是怎样工作的，并有能力质疑实现和继续开发。 牢记 What I cannot create, I do not understand。关键机制要讲到能够据此重建行为、判断代码对错的程度，不能停在“做了校验”“更加可靠”这样的概括。
请像一位理解业务和代码的同事带我看懂项目：用大白话、具体例子和流程图建立连贯理解，在需要背景时提供背景，在需要细节时展开细节。内容完整但有主次，覆盖跨仓库依赖及能力的新增、改变和退出。不要用繁琐的业务能力清单、函数目录或重复对照增加阅读负担，也不要为了易读省略影响判断的重要细节。
流程图应成为理解真实运行机制的骨架，与正文共同讲清调用、读写、条件分支和失败恢复。函数节点链接到 GitHub 对应固定版本的文件即可，不要标注行号或函数起止行，方便核对。
鼠标悬停要提供这个函数/流程的解读或者其在流程图里的作用，帮助用户建立短暂闪存心智。解读的目的是在不影响读者的阅读体验的同时提供更详细准确的信息。因此，如果这个函数/流程名已经能很好地表达其意义，就不需要提供悬停解读；图表的悬停说明统一使用页面自定义浮层，取消浏览器原生悬停提示。不悬停也能读懂主线，想深入时能就近获得解释，普通与放大视图的体验一致。
我希望报告可信，也允许被反驳：重要结论有证据可查，区分实际实现、设计意图和推断，区分独立验证与引用他人记录。明确交代未覆盖和证据不足之处，不把后续修复算进本次 PR，也不把代码已合并等同于生产效果已得到证明。
   ```
...
条件性 Issue Readback 和 Owner Decision 的具体流程继续作为实验机制，按
[DEC-008](decisions.md#实验机制重审与退役) 结合实际使用反馈决定保留、调整或取消。
约 3 次实际使用仅是复盘提醒，不是计数门槛；未触发或未改变下一步动作不自动退役。
复盘同时考虑理解与决策收益、风险覆盖和执行成本，不取消必要的授权边界；结论记入已有
Issue 或 PR，不另建跟踪机制。
