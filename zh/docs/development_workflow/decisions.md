...
2026-09-24 修订：以下规则替代固定三次门槛与默认退役条件，其他决策保持不变。

条件性 Issue Readback 和 Owner Decision 的具体流程继续作为实验机制，分别结合实际使用反馈
决定保留、调整或取消。约 3 次实际使用可作为复盘提醒，不是必须计数的门槛；发现负担或缺口
可立即复盘，长期未使用也可直接评估，不为凑次数制造任务。

评价同时考虑理解与决策收益、风险覆盖、是否减少错误实施或无效等待，以及执行成本。
未触发、未改变下一步动作或风险暂未发生，均不能单独作为取消理由。编码前解读即使未改变
方案，也可能帮助 owner 理解并作出有根据的判断。

可退役的是额外步骤、记录或重复检查，不是必要的授权边界；涉及 owner 的实质性取舍仍由
owner 决定。复盘由 owner 根据现有证据决定保留、调整或取消，可沿用 `PROMOTE`、`MODIFY`、
`RETIRE` 表达结论；结论与理由记入相关已有 Issue 或 PR，并同步受影响的流程说明。证据不足
须如实说明，不能把试行状态当作成熟证明。本节不要求另建计数台账、跟踪文件或专门评测。
