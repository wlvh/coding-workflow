...
不做什么、正式接口在哪里，以及何时需要帮助。来源与适用范围按
[AGENTS.md 的 Authority Map](../AGENTS.md#authority-map) 核对。

成熟项目可以将本页收缩为短桥接页：明确原正式文档在相应范围内仍是正文权威，提供导航和
必要补充，已被覆盖的问题直接引用对应章节，不重复改写整份指南。使用说明与对应的已确认
行为保持一致，不把内部实现、偶然行为或规划写成公开承诺。

## 0. Documentation and Interface Entrypoints

<!-- project-fill: 在此集中维护已有正式用户文档及真实 API、CLI 或协议参考的目录，供 interact.md 等文档引用；注明各自覆盖范围和本页补充内容，区分经核实不适用的入口、适用但尚未配置的参考和证据不足而尚未确认的内容；完成后删除此 marker -->
