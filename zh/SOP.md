...
`SOP.md` 只保存稳定流程入口，不复制易漂移的命令、测试清单或发布细节。发生冲突时，
描述性事实核对当前实现证据，规范性政策核对有效项目授权与政策依据，混合语句拆开处理；
按 `AGENTS.md` 中各来源的适用范围和既有冲突处理规则核对，不以文件名自动覆盖既有权威。
执行记录的保存方式与保留期限遵循本项目真实的审计、可恢复性和交付政策。
...
模型一致。需要超出当前授权的权限、实质性公共语义决定或外部协调时，按既有职责升级，
不猜测或绕过。
