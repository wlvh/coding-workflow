...
一个 canonical 测试入口或命令只在一个权威位置定义；其他章节引用其名称或权威位置，不复制
易漂移的完整命令。项目存在多条受支持测试路径时，分别说明目标受众、环境前提、实际覆盖
范围，以及哪一条产生维护者、CI、PR 或 publication 的正式证据。
...
新增长期测试前，按以下顺序选择足够且最小的覆盖：

1. 复用已有测试；
2. 参数化扩展已有 contract 或 scenario；
3. 扩展现有端到端组合路径；
4. 最后才新增测试函数、测试文件、fixture family 或 runner。

只有能够复现已确认缺陷、保护已登记架构不变量，或覆盖现有测试无法覆盖的真实边界时，
才把对抗探针晋升为长期测试。其他一次性取证探针保留在 PR review 证据中，不进入永久
测试套件。一个失效类别应优先形成一个一般规则和参数化覆盖，不得为每个相邻实例机械增加
独立测试。

跨模块、跨持久化边界或包含外部副作用的改动，必须指出哪一条 scenario 覆盖真实组合路径；
