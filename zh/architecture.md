...
本文档说明其覆盖范围内的当前系统结构；已有正式规范与已接受政策继续在各自范围内有效，
不因本文档的文件名而被覆盖。新增或删除运行入口、模块边界、调用链、数据契约、状态、
错误模型、外部依赖、认证、配置、artifact、副作用、扩展点或架构债务时，必须更新或确认
对应章节。描述性事实核对当前实现、配置、测试、committed artifacts 或可重复运行证据；
规范性约束核对有效的项目授权和政策依据，混合语句拆开处理。

根据项目最难理解的结构选择普通文字、职责表、数据流、状态迁移或对象生命周期等表达；这些
形式均非必填，只有能消除具体歧义时才使用。
...
<!-- project-fill: 写入有实现证据或已接受政策依据的架构不变量，区分当前行为与规范约束并标明适用范围；没有适用不变量时写 Not applicable — 已验证原因；尚未确认时说明已检查范围与证据缺口；完成后删除此 marker -->
...
本节只记录根据项目证据确认、且容易被遗漏的传播关系。
用于指导调查，不要求每次改动执行所有检查。

| 变更类型 | 需要继续调查的影响面 | 相应验证 |
|---|---|---|

<!-- project-fill: 根据本项目的实现、调用关系和已有验证，填写少量重要传播关系；没有适用关系时说明已检查范围与原因；完成后删除此 marker -->
