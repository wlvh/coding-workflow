...
9. **如果 review 有问题，先验证问题是否真实存在，再决定是否修**
   ...
   ```text
   先不动代码，先检查实习生给出的问题是否存在，如存在请列出证据并分析是否值得增加复杂度来修复，并评估实习生的建议是否最优。如果不是，给出你的方案。实习生的发现：《》
   ```
...
- 不要只写结论，要写证据链
   ...
   ```text
   根据你的观察，AGENTS.md 及其内联的文档有没有需要更新的地方？综合评估当前代码分支这个 PR 以什么方式完成了什么任务，给这个项目带来了什么改变和影响，下一步的未来展望是什么？
   ```
